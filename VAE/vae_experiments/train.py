...
def train_epoch(model, optimizer, device, train_loader, epoch, epochs):
    ...
    for x in progress_bar:
        ...
        progress_bar.set_description(f"Train epoch {epoch + 1}/{epochs} KDL Loss: {kdl_loss.item():.4f}, Reproduction Loss: {reproduction_loss.item():.4f}")
...
def plot_good_and_bad_samples(val_loader, model, device, num_samples_to_visualize, experiment_name, plot_dir):
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            for i, (comb_l, kld_l, rec_l) in enumerate(zip(combined_loss, kdl_loss, reconstruction_loss)):
    ...
    for comb_loss, kdl_loss, rec_loss, _, x, x_hat in selected_samples:
        ...
        visualize_comparison(x_flat, x_hat_flat, experiment_name, plot_dir, kdl_loss, rec_loss, comb_loss)
